...
df_115 <- cel %>% filter(congress=='115')
...
df_115 <- rename(df_115, "Gender" = "female")
df_115$Gender <- recode(df_115$Gender, `0` = "Male", `1` = "Female")
ggplot(data = df_115, aes(x = dwnom1, y = all_pass, color = Gender)) +
  geom_point() +
  xlab("Ideology") +
  ylab("Bills Passed")
...
####PUT YOUR CODE HERE
df_115$majority <- recode(df_115$majority, `0` = "Minority", `1` = "Majority")


ggplot(data = df_115, aes(x = votepct, y = all_pass, color = Gender)) +
  geom_point() +
...
ggplot(data = df_115, aes(x = majority, y = les)) +
  geom_boxplot() +
